...
function is_valid_auxtag(x::UInt8, y::UInt8)
    ...
    return (in(x, upper) | in(x, lower)) & (in(y, digit) | in(y, upper) | in(y, lower))
end
...
function try_auxtag(a::UInt8, b::UInt8)::Union{Nothing, AuxTag}
    return is_valid_auxtag(a, b) ? AuxTag(unsafe, a, b) : nothing
end
...
function try_auxtag(str::AbstractString)
    ...
    return try_auxtag(Char(x)::Char, Char(y)::Char)
end
